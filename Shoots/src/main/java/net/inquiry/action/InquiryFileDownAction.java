...
public class InquiryFileDownAction implements Action {
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp)
		...
		//이 부분이 한글 파일명이 깨지는것을 방지해줌.
		String sEncoding = new String (fileName.getBytes("utf-8"), "ISO-8859-1");
		System.out.println(sEncoding);
		
		/*
		 Content-Disposition : attachment : 브라우저에서 다운로드하기 위해 사용됨.
		 */
		...
		{
			int numRead;
			//read(b, 0, b.length) : 바이트 배열 b의 0번부터 b.length 크기만큼 읽어옴.
			while ((numRead = in.read(b,0,b.length)) != -1) { //읽을 데이터가 있는 경우
