...
public class BusinessUserFilter implements Filter {

	////filter가 생성될 때 초기화 시 사용
	public void init(FilterConfig filterConfig) throws ServletException {
		System.out.println("BusinessUserFilter");
	}
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		System.out.println("BusinessUserFilter 확인 시작 - 5");
		...
		if(userClassification ==null || !userClassification.equals("business")) {
			hpresp.setContentType("text/html; charset=UTF-8"); // HTML 응답으로 설정
		    PrintWriter out = hpresp.getWriter();
		    
		    out.println("<script>");
		    out.println("alert('기업회원으로 로그인 해주세요');");
		...
		chain.doFilter(req, resp);
		System.out.println("BusinessUserFilter 끝 - 6");
	}
